...
class Reg
{
  ...
  float integral = 0.0; // Накопленный интеграл

public:  
    Reg(){};
    Reg(float kp, float ki, float kd, float dz, float final)
    : KP(kp), KI(ki), KD(kd), DeathZone(dz), Final(final)
    {};
    ...
    float upd(float curr){
        ...
        // Сброс интеграла при достижении центра стола
...
void loop() {
  ...
  angle_y = constrain(CENTER_ANGLE_Y + out_y * -1, MIN_ANGLE_Y, MAX_ANGLE_X);
